#ifndef __X10_UTIL_SET_H
#define __X10_UTIL_SET_H
...
template<class FMGL(T)> class Set   {
    ...
    static void _serialize(x10aux::ref<x10::util::Set<FMGL(T)> > this_,
...
#endif // X10_UTIL_SET_H

namespace x10 { namespace util { 
template<class FMGL(T)> class Set;
} } 

#ifndef X10_UTIL_SET_H_NODEPS
#define X10_UTIL_SET_H_NODEPS
#include <x10/lang/Any.h>
#include <x10/util/Collection.h>
#ifndef X10_UTIL_SET_H_GENERICS
#define X10_UTIL_SET_H_GENERICS
#endif // X10_UTIL_SET_H_GENERICS
#ifndef X10_UTIL_SET_H_IMPLEMENTATION
#define X10_UTIL_SET_H_IMPLEMENTATION
#include <x10/util/Set.h>


#include "x10/util/Set.inc"

template<class FMGL(T)> x10aux::RuntimeType x10::util::Set<FMGL(T)>::rtt;
template<class FMGL(T)> void x10::util::Set<FMGL(T)>::_initRTT() {
    const x10aux::RuntimeType *canonical = x10aux::getRTT<x10::util::Set<void> >();
    if (rtt.initStageOne(canonical)) return;
    const x10aux::RuntimeType* parents[2] = { x10aux::getRTT<x10::lang::Any>(), x10aux::getRTT<x10::util::Collection<FMGL(T)> >()};
    const x10aux::RuntimeType* params[1] = { x10aux::getRTT<FMGL(T)>()};
    x10aux::RuntimeType::Variance variances[1] = { x10aux::RuntimeType::invariant};
    const char *baseName = "x10.util.Set";
    rtt.initStageTwo(baseName, 2, parents, 1, params, variances);
}
#endif // X10_UTIL_SET_H_IMPLEMENTATION
#endif // __X10_UTIL_SET_H_NODEPS
