	...
	static {
		...
		TRANSFORMATIONS.put("rta", new Transformation("rta",
			...
			protected void performOptimization(final Graph graph) {
				...
				for (final Type type : Program.getTypes()) {
					if (type instanceof ClassType) {
						final ClassType klass = (ClassType)type;
						if (firm.bindings.binding_typerep.is_frame_type(klass.ptr) == 0) {
							/* ignore frame types, seems wrong that they are classes */
							final String classname = klass.getName();

							//assert (!types.containsKey(classname) || types.get(classname).equals(klass));i
							// TODO currently there are two of the basic FIRM classes like GlobalType
				...
				// fetch methods
				final Map<String, Entity> methods = new HashMap<String, Entity>();
				for (final Graph g : Program.getGraphs()) { // take all methods with graph
					final Entity entity = g.getEntity();
					assert (entity.getType() instanceof firm.MethodType);
					/* use ldname to identify methods, normal entity name can be
						 ambiguous even with owner classname as prefix (e.g. instantiations of
						 generic methods in GlobalType) */
					final String name = entity.getLdName();
					/* using ldname when only looking on methods with graph should
						 ensure we don't get duplicates */
					assert (!methods.containsKey(name) || methods.get(name).equals(entity));
					methods.put(name, entity);
				}

				final List<Entity> entrypoints = new LinkedList<Entity>();
				findEntryPoints(methods, entrypoints);

				final List<Type> liveclasses = new LinkedList<Type>();
				findInitialLiveClasses(types, liveclasses);


				final Entity[] initialEntryPoints = entrypoints.toArray(new Entity[entrypoints.size()]);
				final Type[] initialLiveClasses = liveclasses.toArray(new Type[liveclasses.size()]);
			 ...
			 * But only x10.lang.String, x10.io.OutputStreamWriter and
			 * x10.io.InputStreamReader seem to be instantiated (via x10_alloc_object)
			 * in the native runtime code.

			 * X10 struct types seem to get boxed for dynamic method calls (when
			 * cast to Any or some other interface, otherwise calls are statically
			 * bound from the beginning). Normal detection via VptrIsSet should work.

			 * x10.array.Array[x10.lang.String] is used in native main, but
			 * actually created in X10 code in x10.array.Array.makeStringArray. It
			 * could be left out if x10.array.Array.makeStringArray is analyzed (which
			 * currently is not guaranteed because of problems with entity linkage).

			 * There are no reflection classes like in Java, so no "Class" needed.
			 * "Object" is in X10 "Any" which is an interface. A real common     .
			 * super class for class types seem to exist only behind the scenes  .
			 * (although things like "super.toString()" don't work to access)    .

			 * In deserialization code objects of any kind are (re)created. This
			 * is basically the kind of thing RTA will never be able to handle. But
			 * in this X10 implementation the serialization might be only used by the
			 * runtime system to send and receive objects between places in the context
			 * of one program, so this can hopefully be ignored. (Transfering objects
			 * between different programs using serialization would be like dynamic
			 * loading of classes.)

			 * constructors of the initial live classes: Mostly when X10 objects
			 * are created in the C runtime code, constructors aren't called for
			 * some reason. If an constructor would be called there, it would
			 * analyzed from there as long as the runtime code is compiled with
			 * the program. So, no need to add constructors as initially live.

			 * @param types
			 * @param liveclasses
			 */
			private void findInitialLiveClasses(final Map<String, Type> types, final List<Type> liveclasses) {
				final Type string = types.get("x10.lang.String");
				assert (string != null);
				liveclasses.add(string);
				final Type stringArray = types.get("x10.array.Array[x10.lang.String]");
				assert (stringArray != null);
				liveclasses.add(stringArray);
				final Type outputStreamWriter = types.get("x10.io.OutputStreamWriter");
				assert (outputStreamWriter != null);
				liveclasses.add(outputStreamWriter);
				final Type inputStreamReader = types.get("x10.io.InputStreamReader");
				assert (inputStreamReader != null);
				liveclasses.add(inputStreamReader);
				final Type lock = types.get("x10.util.concurrent.Lock");
				assert (lock != null);
				liveclasses.add(lock);
				final Type signal = types.get("x10.util.concurrent.Signal");
				assert (signal != null);
				liveclasses.add(signal);
			}

			/**
			 * Biggest problem is that in some cases entity linkage in FIRM
			 * graphs is incorrect due to duplicate entities. This seems to happen to
			 * functions declared extern in native runtime code.
			 ...
			 * In an attempt to solve this, these functions need to be given as
			 * additional entry points. Problem is that this is probably incomplete
			 * and depends on the native code not changing. Also it could be platform
			 * dependent, for octopos/irtss could need a different set of entry points.

			 * static initializers:
			 * The C main function ("main") calls x10_static_initializer which
			 * is probably generated by the compiler. X10 does not have real static
			 * sections, so no real class initializer functions. Static fields are
			 * initialized by compiler-generated static field getters when accessed for
			 * the first time per place.

			 * Methods common to all classes via the interface "Any" (typeName,
			 * toString, equals, hashCode) are each mapped to a common native
			 * implementation by giving them a common linkername. RTA currently can't
			 * follow such redirections and doesn't find them. So they should be added
			 * to entry points to be safe.

			 * Struct types have their own generated versions of typeName,
			 * toString, equals and hashCode that are always statically bound. If
			 * necessary struct types will be boxed and the box classes work like usual
classes.

			 * @param methods
			 * @param entrypoints
			 */
			private void findEntryPoints(final Map<String, Entity> methods, final List<Entity> entrypoints) {
				Entity main = methods.get("main");
				if (main == null) {
					/* workaround because CompilerOptions can't be queried from here */
					// octopos/irtss
				...
				/* because of broken references happening to functions declared
					 extern in runtime C-code but implemented in X10 or generated by compiler */
				final Entity x10main = methods.get("x10_main");
				assert (x10main != null);
				assert (x10main.getGraph() != null);
				entrypoints.add(x10main);
				final Entity staticInit = methods.get("x10_static_initializer");
				assert (staticInit != null);
				assert (staticInit.getGraph() != null);
				entrypoints.add(staticInit);
				final Entity getStaticMonitor = methods.get("_ZN3x108compiler14InitDispatcher17get_staticMonitorEv");
				assert (getStaticMonitor != null);
				assert (getStaticMonitor.getGraph() != null);
				entrypoints.add(getStaticMonitor);
				final Entity makeStringArray = methods.get("_ZN3x105array5Array15makeStringArrayEPvi");
				assert (makeStringArray != null);
				assert (makeStringArray.getGraph() != null);
				entrypoints.add(makeStringArray);
				final Entity printException =
					methods.get("_ZN3x104lang7Runtime14printExceptionEPN3x104lang16CheckedThrowableE");
				...
				final Entity typeName = methods.get("x10_object_8typeNamev");
				assert (typeName != null);
				assert (typeName.getGraph() != null);
				entrypoints.add(typeName);
				final Entity toString = methods.get("x10_object_8toStringv");
				assert (toString != null);
				assert (toString.getGraph() != null);
				entrypoints.add(toString);
				final Entity equals = methods.get("x10_object_6equalsPN3x104lang3AnyE");
				assert (equals != null);
				assert (equals.getGraph() != null);
				entrypoints.add(equals);
				final Entity hashCode = methods.get("x10_object_8hashCodev");
	...
	public static void disableAllOptimizations() {
		for (final Transformation opt : TRANSFORMATIONS.values()) {
