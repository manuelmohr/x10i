			...
			protected void performOptimization(final Graph graph) {
				...
				/*
					Biggest problem is that in some cases entity linkage in FIRM graphs is incorrect due to duplicate entities. This seems to happen to functions declared extern in native runtime code.
						...
						Problem is that this is probably incomplete and depends on the native code not changing.

					static initializers:
					The C main function ("main") calls x10_static_initializer which is probably generated by the compiler.
					X10 does not have real static sections, so no real class initializer functions. Static fields are initialized by compiler-generated static field getters when accessed for the first time.

					Nethods common to all classes via the interface "Any" (typeName, toString, equals, hashCode) are each mapped to a common native implementation by giving them a common linkername. RTA currently can't follow such redirections and doesn't find them. So they should be added to entry points to be safe.
				...
				*/
				...
				/*
					...
					There are no reflection classes like in Java, so no "Class".
				...
				*/
				...
				Entity[] initialEntryPoints = { main, x10main, staticInit, makeStringArray, getStaticMonitor, printException, typeName, toString, hashCode, equals };
				Type[] initialLiveClasses = { string, stringArray, outputStreamWriter, inputStreamReader, lock, signal };
