 ...
 * @author Julian Oppermann, Matthias Braun
 *
 * Runtime (here)
 * --------------
 * The runtime defines the type "serialization_buffer_t", consisting of an
 * obstack "obst" that stores "bytes_written" bytes of serialized data and
 * table of x10_objects "serialized_object" where references to already
 * serialized objects are remembered (this is needed to break cycles).
 *
 * A new serialization buffer should be setup by calling
 * x10_init_serialization_buffer().
 *
 ...
 *
 * If "objPtr" is NULL, the value
 * X10_SERIALIZATION_NULL_TYPE_UID = (uint32_t)0 is appended to the buffer.
 ...
 * X10_SERIALIZATION_KNOWN_OBJECT_TYPE_ID = (uint32_t)-1, followed by "idx" to
 * the buffer.
 ...
 * "deserialized_objects", which is used in the reference-cycle breaking
 * mechanism.
 *
 * A new deserialization buffer is acquired by calling
 * "x10_deserialization_init(const char *data, size_t length)".
 *
 * A call to "x10_deserialization_restore_primitive(
 * deserialization_buffer_t *buf, void *addr, size_t nbytes)" copies "nbytes"
 * of "buf"'s data to the location references by "addr" and advances
 * "buf->cursor" accordingly.
 *
 * "x10_deserialization_restore_object(deserialization_buffer_t *buf,
 * x10_object **addr)" allocates and initializes a class instance:
 *
 ...
 * If the id equals X10_SERIALIZATION_KNOWN_OBJECT_TYPE_ID, the object has
 * already been deserialized from "buf". We read the index from the byte
 * sequence and lookup the reference from "buf->deserialized_objects". The
 * reference is then written to "*addr".
 ...
 * firmType)" (both parameters represent the same class, but we need the
 * Polyglot variant for easy name mangling and subtype checks). For X10
 * classes, the serialize method has to be bound dynamically. The serialize
 * method for structs is always called directly, therefore the method is bound
 * statically. The deserialize method is either called directly, or via a
 * lookup in the table "__deserialize_methods", thus it is not part of the
 * vtable. The type uid is set arbitrarily by this (setup) method.
 *
 ...
 * by the type uid and contains for each class the function pointer to the
 * type's deserialize method and the pointer to the type's vtable.
 *
 * Example
 * -------
 * Assume we have
 ...
 * -----------
 *
 * - the "toplevel" value to be serialized has to be a class instance, because
 *   we need to perform a dynamic dispatch to find the serializer method, and a
 *   runtime typeinfo is required (which only classes have) to lookup the
 *   deserializer method.
